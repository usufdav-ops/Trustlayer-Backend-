...
-- Safe to re-run: adds the verification columns to a pre-existing users table too.
...
/* ============================================================
   Email (verification) — generic SMTP via nodemailer
   ============================================================ */

// Works with any SMTP provider: Resend, SendGrid, Mailgun, Postmark, Gmail
// (app password), AWS SES SMTP, etc. Just point the SMTP_* env vars at it.
...
const MAIL_FROM = process.env.SMTP_FROM || 'TrustLayer <no-reply@trustlayer.app>';
...
async function sendVerificationEmail(toEmail, fullName, rawToken) {
  const verifyLink = `${APP_URL}/api/auth/verify-email?token=${rawToken}`;

  if (!SMTP_CONFIGURED) {
    // No SMTP configured (e.g. local dev) — log the link so it's still usable.
    console.warn(`[email] SMTP not configured. Verification link for ${toEmail}: ${verifyLink}`);
  ...
  try {
    await mailer.sendMail({
      from: MAIL_FROM,
      to: toEmail,
      subject: 'Verify your TrustLayer email',
      text: `Hi ${fullName || 'there'},\n\nVerify your email to activate your TrustLayer account:\n${verifyLink}\n\nThis link expires in 24 hours. If you didn't sign up, ignore this email.`,
      html: `<p>Hi ${fullName || 'there'},</p><p>Verify your email to activate your TrustLayer account:</p><p><a href="${verifyLink}">${verifyLink}</a></p><p>This link expires in 24 hours. If you didn't sign up, ignore this email.</p>`,
    });
    return { sent: true };
  } catch (err) {
...
authRouter.post('/signup', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (account_type, full_name, email, dial_code, phone_number, country, password_hash, verification_token_hash, verification_token_expires)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id, account_type, full_name, email, country, email_verified, created_at`,
      [accountType, String(fullName).trim(), normalizedEmail, dialCode || null, String(phoneNumber).trim(), country, passwordHash, tokenHash, expires]
    );

    const user = result.rows[0];
    const token = signToken(user);
    const emailResult = await sendVerificationEmail(user.email, user.full_name, rawToken);

    return res.status(201).json({
      token,
      user: { id: user.id, accountType: user.account_type, fullName: user.full_name, email: user.email, country: user.country, emailVerified: user.email_verified },
...
authRouter.get('/me', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, account_type, full_name, email, country, email_verified, created_at FROM users WHERE id = $1',
      [req.user.id]
    );
    const user = result.rows[0];
    ...
    return res.json({
      user: {
        ...
        email: user.email, country: user.country, emailVerified: user.email_verified, createdAt: user.created_at,
      },
    });
  } catch (err) {
...
// Clicked directly from the email, so it returns an HTML page, not JSON.
authRouter.get('/verify-email', async (req, res) => {
  const { token } = req.query;
  const sendPage = (title, message, ok) => res.status(ok ? 200 : 400).send(`
    <!DOCTYPE html><html><head><meta charset="utf-8"><title>${title}</title>
    <style>body{font-family:system-ui,sans-serif;max-width:480px;margin:80px auto;text-align:center;color:#1a1a1a}
    h1{font-size:1.3rem}p{color:#555}</style></head>
    <body><h1>${title}</h1><p>${message}</p></body></html>
  `);

  if (!token || typeof token !== 'string') {
    return sendPage('Invalid link', 'This verification link is missing its token.', false);
  }

  try {
    const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
    const result = await pool.query(
      `SELECT id, email_verified, verification_token_expires FROM users WHERE verification_token_hash = $1`,
      [tokenHash]
    );
    const user = result.rows[0];

    if (!user) {
      return sendPage('Invalid or expired link', 'This verification link is invalid. Please request a new one from the app.', false);
    }
    if (user.email_verified) {
      return sendPage('Already verified', 'Your email is already verified — you can close this tab and return to the app.', true);
    }
    if (new Date(user.verification_token_expires) < new Date()) {
      return sendPage('Link expired', 'This verification link has expired. Please request a new one from the app.', false);
    }
    ...
    return sendPage('Email verified', 'Your email has been verified. You can close this tab and return to the app.', true);
  } catch (err) {
    console.error('verify-email error', err);
    return sendPage('Something went wrong', 'Please try again in a moment.', false);
  }
});
...
authRouter.post('/resend-verification', async (req, res) => {
  try {
    ...
    const normalizedEmail = String(email).trim().toLowerCase();
    const result = await pool.query('SELECT id, full_name, email, email_verified FROM users WHERE email = $1', [normalizedEmail]);
    const user = result.rows[0];
    ...
    return res.status(500).json({ error: 'Something went wrong resending the verification email' });
  }
});
